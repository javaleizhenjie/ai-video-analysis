...
class ZLMediaKit():
    ...
    def addStreamPusherProxy(self,app,name,schema="rtsp", vhost="__defaultVhost__",dst_url=None,retry_count=60,timeout_sec=10):
        # 官方文档地址：https://github.com/zlmediakit/ZLMediaKit/wiki/MediaServer%E6%94%AF%E6%8C%81%E7%9A%84HTTP-API
        __key = None
        #key返回示例1 __defaultVhost__/proxy/0  //流的唯一标识
        #key返回示例2 rtsp/__defaultVhost__/live/cam25959636e0/3c17914f8885422e4ebb84b602b48e28

        __msg = "开启转推失败|dst_url=%s" % dst_url
        ...
        try:
            ...
            params = {
                ...
                'app': app,
                'stream': name,
                'schema': schema,  # 转推到dst的协议，例如 rtsp或rtmp
                'dst_url': dst_url, # 转推目标url，带参数需要自行url转义
                "timeout_sec":timeout_sec,#推流超时时间，单位秒，float类型
            }
            if schema == "rtsp":
                params["rtp_type"] = 0 # rtsp转推时，推流方式，0：tcp，1：udp
            if retry_count > 0:
                params["retry_count"] = retry_count # 转推失败重试次数，默认无限重试
